...
<template>
  <header class="fixed top-0 left-0 right-0 z-50 border-b transition-colors duration-500 backdrop-blur-md"
    ...
    <div class="flex items-center justify-between px-6 sm:px-10 lg:px-12 py-5">
      ...
      <nav class="hidden md:flex items-center gap-9">
        <NuxtLink v-for="link in items" :key="link.to" :to="link.to" :data-tour="`nav-${link.label.toLowerCase()}`"
          ...
          {{ link.label }}
          <span v-if="isActive(link.to)" class="absolute left-0 right-0 -bottom-0.5 h-px bg-[#B5563A]" />
        </NuxtLink>
      ...
      <div class="flex md:hidden items-center gap-2">
        <button class="w-10 h-10 flex flex-col items-center justify-center gap-[5px]" :aria-expanded="open"
          aria-label="Menu" @click="open = !open">
          <span class="block w-5 h-px bg-[#1F1B16] transition-transform duration-300"
    ...
    <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="opacity-0 -translate-y-2"
      ...
      <nav v-if="open" class="md:hidden border-t border-[#DDD6C9] bg-[#F7F4EF] px-6 py-4 flex flex-col">
        <NuxtLink v-for="link in items" :key="link.to" :to="link.to"
