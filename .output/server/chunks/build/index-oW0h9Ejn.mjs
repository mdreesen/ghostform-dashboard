...
import { ref, unref, defineComponent, withCtx, createVNode, createTextVNode, mergeProps, useAttrs, computed, toRef, h, resolveDynamicComponent, useSSRContext } from 'vue';
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (__props.data.length > 0) {
        _push(`<div class="text-center pt-2"><span>What home is this for?</span><select id="status-select" class="w-full rounded-xl border border-gray-600 bg-gray-700/50 py-3 px-4 text-lg text-white shadow-lg transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500"><option disabled value=""${ssrIncludeBooleanAttr(Array.isArray(unref(address)) ? ssrLooseContain(unref(address), "") : ssrLooseEqual(unref(address), "")) ? " selected" : ""}>Choose home</option><!--[-->`);
      ...
      _push(`</div></div><div class="flex sm:items-center gap-3 shrink-0 relative z-10 flex-row md:flex-col lg:flex-row w-full md:w-auto border-t border-slate-800/50 md:border-none pt-4 md:pt-0">`);
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(formFunnels.value, (item) => {
        ...
        _push(ssrRenderComponent(_component_baseCardForm, {
          ...
          data: item.id === "open-house" && unref(home)
        }, null, _parent));
        _push(`</div>`);
      });
      _push(`<!--]--></div></div></div>`);
...
//# sourceMappingURL=index-oW0h9Ejn.mjs.map
