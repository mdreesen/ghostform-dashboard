...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const analysis = computed(() => lead.value?.analysis || null);
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (!unref(pending_data)) {
        ...
        _push(`</div></div></header><section class="gf-depth mb-20"><div class="flex items-baseline gap-4 border-b border-[#DDD6C9] pb-3.5 mb-8"><span class="gf-eyebrow">Qualifying</span><span class="font-display text-[25px] font-semibold tracking-tight">How serious are they?</span></div>`);
        if (!unref(qual).sentAt && !unref(qual).completedAt) {
          ...
          _push(`<div>`);
          if (unref(analysis)) {
            _push(`<div class="grid sm:grid-cols-3 gap-px bg-[#DDD6C9] border border-[#DDD6C9] mb-8"><div class="bg-[#F7F4EF] p-7"><p class="gf-eyebrow mb-3">Readiness</p><p class="font-display text-[42px] font-semibold leading-none tabular-nums">${ssrInterpolate(unref(analysis).readiness)}<span class="text-[18px] text-[#A9A39A]">/100</span></p><p class="text-[13px] text-[#8A847C] mt-2">${ssrInterpolate(unref(analysis).readinessLabel)}</p></div><div class="bg-[#F7F4EF] p-7"><p class="gf-eyebrow mb-3">Financing</p><p class="${ssrRenderClass([unref(analysis).financingRisk === "high" ? "text-[#B5563A]" : "", "font-display text-[26px] font-semibold capitalize"])}">${ssrInterpolate(unref(analysis).financingRisk)}</p><p class="text-[13px] text-[#8A847C] mt-2">${ssrInterpolate(unref(analysis).financingRisk === "high" ? "Most likely thing to stall this" : unref(analysis).financingRisk === "low" ? "Not a concern" : "Worth confirming")}</p></div><div class="bg-[#F7F4EF] p-7"><p class="gf-eyebrow mb-3">Answered</p><p class="font-display text-[42px] font-semibold leading-none tabular-nums">${ssrInterpolate(unref(answeredCount))}</p><p class="text-[13px] text-[#8A847C] mt-2">questions completed</p></div></div>`);
          } else {
            _push(`<!---->`);
          }
...
//# sourceMappingURL=details-CaWGLb7U.mjs.map
