...
import { ssrRenderAttrs, ssrRenderComponent, ssrRenderStyle, ssrInterpolate, ssrIncludeBooleanAttr, ssrLooseContain, ssrLooseEqual, ssrRenderClass, ssrRenderList, ssrRenderAttr } from 'vue/server-renderer';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    computed(() => lead.value?.qualification.answers);
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (!unref(pending_data)) {
        ...
        _push(`</div></div></header><section class="gf-depth mb-20"><div class="flex flex-col items-baseline gap-4 border-b border-[#DDD6C9] pb-3.5 mb-8"><div class="flex items-baseline gap-4 border-b border-[#DDD6C9] pb-3.5 mb-8"><span class="gf-eyebrow">Qualifying</span><span class="font-display text-[25px] font-semibold tracking-tight">How serious are they?</span></div>`);
        if (unref(lead).qualification) {
          _push(`<div class="flex flex-col gap-4"><div class="flex flex-col"><span class="gf-eyebrow">Lead is looking to ${ssrInterpolate(unref(lead).qualification.intent)}</span><span class="font-display text-[25px] font-semibold tracking-tight">Qualifying Details</span></div><div><div><p>${ssrInterpolate(`Time Line ${unref(lead).qualification?.answers?.q_timeline}`)}</p><p>${ssrInterpolate(`Financing ${unref(lead).qualification?.answers?.q_financing}`)}</p><p>${ssrInterpolate(`Lender ${unref(lead).qualification?.answers?.q_lender}`)}</p><p>${ssrInterpolate(`Budget $${unref(lead).qualification?.answers?.q_budget_max}`)}</p><p>${ssrInterpolate(`Must Haves ${unref(lead).qualification?.answers?.q_must_haves}`)}</p><p>${ssrInterpolate(`Deal Breakders ${unref(lead).qualification?.answers?.q_deal_breakers}`)}</p><p>${ssrInterpolate(`Current Situation ${unref(lead).qualification?.answers?.q_current_situation}`)}</p><p>${ssrInterpolate(`Areas they are considering ${unref(lead).qualification?.answers?.q_areas}`)}</p></div></div><div class="flex flex-wrap items-center gap-3"><button${ssrIncludeBooleanAttr(unref(analysing)) ? " disabled" : ""} class="px-6 py-3.5 border border-[#DDD6C9] text-[#8A847C] text-[11px] uppercase tracking-[0.12em] font-semibold hover:border-[#1F1B16] hover:text-[#1F1B16] transition-colors disabled:opacity-40">${ssrInterpolate(unref(analysing) ? "Thinking…" : unref(analysis) ? "Re-run analysis" : "Run analysis")}</button>`);
...
//# sourceMappingURL=details-C0or7yNB.mjs.map
