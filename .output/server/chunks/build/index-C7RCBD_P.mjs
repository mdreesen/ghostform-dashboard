...
import { _ as __nuxt_component_1 } from './Navigate-nQh8qSK4.mjs';
import { _ as _sfc_main$4 } from './Modal-BRXgObOb.mjs';
import { I as useNuxtData, q as _sfc_main$8, _ as __nuxt_component_0$2, v as reactivePick, J as useRuntimeConfig } from './server.mjs';
import { _ as __nuxt_component_1$1 } from './Section-DcEM7TqR.mjs';
import { ref, defineComponent, withCtx, createVNode, unref, openBlock, createBlock, withDirectives, isRef, Fragment, renderList, toDisplayString, vModelSelect, createCommentVNode, createTextVNode, mergeProps, useAttrs, computed, toRef, h, resolveDynamicComponent, useSSRContext } from 'vue';
import { ssrRenderAttrs, ssrRenderComponent, ssrRenderList, ssrRenderAttr, ssrInterpolate, ssrRenderClass, ssrIncludeBooleanAttr, ssrLooseContain, ssrLooseEqual, ssrRenderVNode } from 'vue/server-renderer';
import { y as defu } from '../nitro/nitro.mjs';
import { encode } from 'uqr';
import './Auth-YSaql3Au.mjs';
import './utils-BeoGzRrR.mjs';
import 'aria-hidden';
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<!--[--><div class="absolute inset-0 bg-radial from-slate-800/10 via-transparent to-transparent opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-300"></div><div class="flex items-start gap-4 max-w-2xl relative z-10"><div class="p-3 bg-slate-900 border border-slate-800 rounded-xl text-slate-400 group-hover:text-cyan-400 group-hover:border-slate-700 transition-colors shrink-0 hidden sm:block"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round"${ssrRenderAttr("d", __props.icon)}></path></svg></div><div class="space-y-1.5"><div class="flex items-center gap-3"><h3 class="text-base font-black text-white tracking-wide group-hover:text-cyan-400 transition-colors">${ssrInterpolate(__props.label)}</h3><span class="${ssrRenderClass([__props.badgeClass, "text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-md border"])}">${ssrInterpolate(__props.badge)}</span></div><p class="text-xs text-slate-400 leading-relaxed font-medium">${ssrInterpolate(__props.description)}</p></div></div><div class="flex sm:items-center gap-3 shrink-0 relative z-10 flex-row md:flex-col lg:flex-row w-full md:w-auto border-t border-slate-800/50 md:border-none pt-4 md:pt-0">`);
        ...
        body: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            if (__props.data.length > 0) {
              _push2(`<div${_scopeId}><select id="status-select" class="w-full rounded-xl border border-gray-600 bg-gray-700/50 py-3 px-4 text-lg text-white shadow-lg transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500"${_scopeId}><option disabled value=""${ssrIncludeBooleanAttr(Array.isArray(unref(address)) ? ssrLooseContain(unref(address), "") : ssrLooseEqual(unref(address), "")) ? " selected" : ""}${_scopeId}>Choose home</option><!--[-->`);
              ssrRenderList(__props.data, (item, index) => {
                _push2(`<option${ssrRenderAttr("value", item.address)}${ssrIncludeBooleanAttr(Array.isArray(unref(address)) ? ssrLooseContain(unref(address), item.address) : ssrLooseEqual(unref(address), item.address)) ? " selected" : ""}${_scopeId}>${ssrInterpolate(item.name ?? item?.address)}</option>`);
              });
              _push2(`<!--]--></select>`);
              if (unref(address)) {
                _push2(`<div class="pt-10"${_scopeId}>`);
                _push2(ssrRenderComponent(_component_baseHeaderSection, {
            ...
            return [
              __props.data.length > 0 ? (openBlock(), createBlock("div", { key: 0 }, [
                withDirectives(createVNode("select", {
                  ...
                  (openBlock(true), createBlock(Fragment, null, renderList(__props.data, (item, index) => {
                    return openBlock(), createBlock("option", {
                      value: item.address,
                      key: index
                    }, toDisplayString(item.name ?? item?.address), 9, ["value"]);
                  }), 128))
                ], 8, ["onUpdate:modelValue"]), [
                  [vModelSelect, unref(address)]
                ]),
                unref(address) ? (openBlock(), createBlock("div", {
                  key: 0,
                  class: "pt-10"
                }, [
                  createVNode(_component_baseHeaderSection, {
                    text: `Chosen Address<br>${unref(address)}`
                  }, null, 8, ["text"])
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(_attrs)}><div class="max-w-4xl mx-auto space-y-8"><header class="border-b border-slate-800/60 pb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">`);
      ...
      _push(`</header><div class="space-y-5"><!--[-->`);
      ssrRenderList(formFunnels.value, (item) => {
        _push(`<div class="relative overflow-hidden bg-slate-900/40 border border-slate-800/80 hover:border-slate-700/80 rounded-2xl p-6 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6 transition-all duration-300 group hover:shadow-2xl">`);
...
//# sourceMappingURL=index-C7RCBD_P.mjs.map
