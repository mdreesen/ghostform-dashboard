...
import { w as reactivePick, L as useRuntimeConfig, _ as __nuxt_component_0, q as _sfc_main$5, u as useHead, K as useNuxtData } from "../server.mjs";
import { _ as _sfc_main$4 } from "./Modal-D1BiXRFq.js";
import { useAttrs, computed, toRef, h, createVNode, resolveDynamicComponent, unref, useSSRContext, defineComponent, mergeProps, ref, withCtx, createTextVNode } from "vue";
import { ssrRenderVNode, ssrRenderComponent, ssrRenderAttr, ssrInterpolate, ssrRenderClass, ssrIncludeBooleanAttr, ssrLooseContain, ssrLooseEqual, ssrRenderList, ssrRenderAttrs, ssrRenderStyle } from "vue/server-renderer";
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (__props.data.length > 0) {
        _push(`<div class="pt-4"><label${ssrRenderAttr("for", `home-${__props.label}`)} class="gf-eyebrow block mb-3">Which property</label><select${ssrRenderAttr("id", `home-${__props.label}`)} class="w-full bg-[#F7F4EF] border border-[#DDD6C9] px-4 py-3.5 text-[15px] text-[#1F1B16] focus:outline-none focus:border-[#B5563A] transition-colors cursor-pointer"><option disabled value=""${ssrIncludeBooleanAttr(Array.isArray(unref(address)) ? ssrLooseContain(unref(address), "") : ssrLooseEqual(unref(address), "")) ? " selected" : ""}>Choose a property…</option><!--[-->`);
        ...
        _push(`<!--]--></select>`);
        if (unref(address)) {
          _push(`<p class="text-[12.5px] text-[#8A847C] mt-2.5"> Leads from will be tagged to ${ssrInterpolate(unref(address))}. </p>`);
        } else {
          _push(`<p class="text-[12.5px] text-[#A9A39A] mt-2.5"> Pick one so you know which listing each lead came from. </p>`);
        }
        _push(`</div>`);
      } else {
        _push(`<div class="pt-4"><p class="text-[12.5px] text-[#A9A39A] leading-relaxed"> No properties added yet. `);
        _push(ssrRenderComponent(_component_NuxtLink, {
          to: "/dashboard/home/create",
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              ...
              return [
                createTextVNode(" Add one ")
              ];
            }
          }),
          _: 1
        }, _parent));
        _push(` to tag leads to a specific listing — or use this form as-is. </p></div>`);
      }
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const activeHomes = computed(
      () => (home.value ?? []).filter((h2) => (h2.status || "active") !== "sold")
    );
...
//# sourceMappingURL=index-BiUXXeiD.js.map
